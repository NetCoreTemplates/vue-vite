...
As a consequence of designing your UI decoupled from your back-end server is that it also becomes considerably 
...
The .NET 6 `/api` backend server is required for this dynamic functions including the Hello Service on the home page
and the [built-in Authentication](https://docs.servicestack.net/auth). 

It still contains a complete version of this App that can be hosted independently where it can either host the 
entire App when served directly from its server:
...
Since hosting on GitHub Pages CDN is free, the only cost is for hosting this Apps .NET Server which is being hosted 
...
reusable message-based APIs where the same System APIs can be used from Web, Mobile & Desktop Apps 
